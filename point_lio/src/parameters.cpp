...
bool prior_pcd_enable = false, prior_pcd_localization_mode = false;
bool prior_pcd_init_pose_configured = false;
std::string prior_pcd_map_path, prior_pcd_map_frame = "map",
  prior_pcd_init_pose_frame = "base_footprint", prior_pcd_lidar_frame = "front_mid360",
  prior_pcd_output_base_frame = "base_footprint";
std::vector<double> prior_pcd_init_pose;
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    ...
    nh->get_parameter("prior_pcd.enable", prior_pcd_enable);
    ...
    nh->declare_parameter<std::string>("prior_pcd.prior_pcd_map_path", "");
    ...
    nh->get_parameter("prior_pcd.init_pose", prior_pcd_init_pose);
    if (prior_pcd_init_pose.size() == 3) {
      prior_pcd_init_pose.resize(6, 0.0);
    } else if (prior_pcd_init_pose.size() != 6) {
      if (!prior_pcd_init_pose.empty()) {
        RCLCPP_WARN(
          nh->get_logger(), "prior_pcd.init_pose must contain 3 or 6 values, got %zu. "
          "Ignoring configured initial pose.",
          prior_pcd_init_pose.size());
      }
      prior_pcd_init_pose.clear();
    }
    prior_pcd_init_pose_configured = !prior_pcd_init_pose.empty();
    ...
    nh->declare_parameter<std::string>("prior_pcd.lidar_frame", "front_mid360");
    nh->get_parameter("prior_pcd.lidar_frame", prior_pcd_lidar_frame);

    nh->declare_parameter<std::string>("prior_pcd.output_base_frame", "base_footprint");
    nh->get_parameter("prior_pcd.output_base_frame", prior_pcd_output_base_frame);
