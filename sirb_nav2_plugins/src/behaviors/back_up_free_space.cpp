...
#include "sirb_nav2_plugins/behaviors/back_up_free_space.hpp"
...
void BackUpFreeSpace::onConfigure()
{
  ...
  end_time_ = clock_->now();
  command_time_allowance_ = rclcpp::Duration(0, 0);
  ...
  nav2_util::declare_parameter_if_not_declared(node, "robot_radius", rclcpp::ParameterValue(0.25));
  ...
  const std::shared_ptr<const BackUpAction::Goal> command)
{
  ...
  pose.theta = yawFromQuaternion(initial_pose_.pose.orientation);
  initial_center_cost_ = 0;
  costAtWorld(pose.x, pose.y, initial_center_cost_);

  // Find the best direction to back up
  float best_angle = findBestDirection(costmap, pose, -M_PI, M_PI, max_radius_, M_PI / 32.0);

  selected_global_angle_ = best_angle;
  const double base_yaw = pose.theta;
  const double base_angle = selected_global_angle_ - base_yaw;
  twist_x_ = std::cos(base_angle) * command->speed;
  twist_y_ = std::sin(base_angle) * command->speed;
...
nav2_behaviors::Status BackUpFreeSpace::onCycleUpdate()
{
  ...
  pose.theta = yawFromQuaternion(current_pose.pose.orientation);
...
{
  const double speed = std::hypot(cmd_vel->linear.x, cmd_vel->linear.y);
  if (speed < 1.0e-4) {
    return true;
  }

  const double remaining = std::fabs(command_x_) - distance;
  const int max_cycle_count = static_cast<int>(cycle_frequency_ * simulate_ahead_time_);
  const double dir_x = std::cos(selected_global_angle_);
  const double dir_y = std::sin(selected_global_angle_);
  bool fetch_data = true;

  for (int cycle_count = 0; cycle_count < max_cycle_count; ++cycle_count) {
    const double sim_position_change = speed * (cycle_count / cycle_frequency_);
    ...
    sim_pose.y = pose2d.y + sim_position_change * dir_y;
    if (!collision_checker_->isCollisionFree(sim_pose, fetch_data)) {
      if (!allow_escape_from_collision_ || !have_latest_costmap_ ||
        distance > escape_collision_max_distance_)
      {
        return false;
      }
      unsigned char sim_center_cost = nav2_costmap_2d::LETHAL_OBSTACLE;
      if (!costAtWorld(sim_pose.x, sim_pose.y, sim_center_cost)) {
        return false;
      }
      if (sim_center_cost > initial_center_cost_ ||
        sim_center_cost >= nav2_costmap_2d::LETHAL_OBSTACLE)
      {
        return false;
      }
      RCLCPP_WARN_THROTTLE(
        logger_, *clock_, 500,
        "BackUpFreeSpace: escaping from occupied footprint, center cost %u -> %u",
        initial_center_cost_, sim_center_cost);
    }
    fetch_data = false;
  }
  return true;
...
bool BackUpFreeSpace::costAtWorld(double wx, double wy, unsigned char & cost) const
{
  if (!have_latest_costmap_ || latest_costmap_.metadata.resolution <= 0.0) {
    return false;
  }
  const auto & meta = latest_costmap_.metadata;
  const double mx_d = (wx - meta.origin.position.x) / meta.resolution;
  const double my_d = (wy - meta.origin.position.y) / meta.resolution;
  if (mx_d < 0.0 || my_d < 0.0) {
    return false;
  }
  const auto mx = static_cast<unsigned int>(mx_d);
  const auto my = static_cast<unsigned int>(my_d);
  if (mx >= meta.size_x || my >= meta.size_y) {
    return false;
  }
  const size_t idx = static_cast<size_t>(my) * meta.size_x + mx;
  if (idx >= latest_costmap_.data.size()) {
    return false;
  }
  cost = static_cast<unsigned char>(latest_costmap_.data[idx]);
  return true;
}
